...
struct LodePNGDecompressSettings {
  ...
  not hard limit nor a guarantee for all decoders, but can prevent excessive memory usage.
  pdvrdt's no-internal-zlib PNG decoder passes the expected filtered scanline size
  through this field to its custom callback, preserving a stricter caller limit.
  Its adapter enforces that bound as well as the program's global safety ceiling.
  Set to 0 for no additional caller limit (the default).*/
