...
int pdvOut(const std::string& IMAGE_FILENAME) {
	...
	constexpr uint8_t SIG_LENGTH = 7;
	...
	constexpr uint8_t 
		MASTODON_CHUNK_SIZE_INDEX_DIFF = 0x04,
		DEFAULT_CHUNK_SIZE_INDEX_DIFF = 0x70,
		MASTODON_PROFILE_DATA_INDEX_DIFF = 0x09,
		DEFAULT_PROFILE_DATA_INDEX_DIFF = 0x0B,
	...
	const uint32_t
		...
		CHUNK_SIZE = isMastodonFile ? getByteValue<uint32_t>(image_vec, CHUNK_SIZE_INDEX) - MASTODON_CHUNK_SIZE_DIFF : getByteValue<uint32_t>(image_vec, CHUNK_SIZE_INDEX);

	uint8_t byte = image_vec.back();

	image_vec.erase(image_vec.begin(), image_vec.begin() + PROFILE_DATA_INDEX);
	image_vec.erase(image_vec.begin() + (isMastodonFile ? CHUNK_SIZE + CHUNK_SIZE_DIFF : CHUNK_SIZE - CHUNK_SIZE_DIFF), image_vec.end());
