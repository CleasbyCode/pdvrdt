...
void decryptFile(std::vector<uchar>&Image_Vec, std::vector<uchar>&File_Vec, std::string& data_file_name) {
	...
	const uint8_t
		...
		data_file_name_length = static_cast<uint8_t>(data_file_name.length());

	uint32_t
		file_size = static_cast<uint32_t>(Image_Vec.size()),
		index_pos = 0;
