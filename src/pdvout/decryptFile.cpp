std::string decryptFile(std::vector<uint8_t>&Image_Vec, uint8_t (&xor_key)[XOR_KEY_LENGTH], std::string& encrypted_data_filename) {

	const uint32_t ENCRYPTED_DATA_FILE_SIZE = static_cast<uint32_t>(Image_Vec.size());

	const uint8_t ENCRYPTED_DATA_FILENAME_LENGTH = static_cast<uint8_t>(encrypted_data_filename.length());
