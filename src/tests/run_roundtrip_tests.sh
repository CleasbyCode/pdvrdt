...
run_capsize_case() {
    ...
    local required_lines=(
        ...
        "Cover Image: 0KB, 128x128, Non-interlaced 8-bit RGB PNG, Adaptive (1,15,4) matrix embedding."
        "Theoretical adaptive capacity limit for this cover image:              1619 bytes (~1KB)."
        "Conservative maximum compressed capacity with a 20-character filename: 1401 bytes (~1KB)."
        "Recommended  maximum compressed capacity with a 20-character filename: 401 bytes (~0KB)."
...
run_reddit_capacity_failure_case() {
    ...
    local required_lines=(
        "Cover Image: 0KB, 128x128, Non-interlaced 8-bit RGB PNG, Adaptive (1,15,4) matrix embedding."
        "Compressed data file (payload) size: 50011 bytes (50KB)."
        "Theoretical adaptive capacity limit for this cover image:              1619 bytes (~1KB)."
        "Conservative maximum compressed capacity with a 20-character filename: 1401 bytes (~1KB)."
        "Recommended  maximum compressed capacity with a 20-character filename: 401 bytes (~0KB)."
        ...
        # conservative figure (1401) and not the recommendation 1000 below it.
        "Compressed payload size of 50011 bytes (50KB) exceeds this cover image's maximum of 1404 bytes (~1KB) for a 17-character filename."
        "Where capacity permits, leave at least 1KB of headroom below that (401 bytes)."
    )
    local expected
    for expected in "${required_lines[@]}"; do
