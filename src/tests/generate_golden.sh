...
CASES=(
    ...
    $'reddit_v2_even_domain\t-r\ttestdata/payloads/payload_text.txt\treddit\t'
    $'reddit_v2_odd_domain\t-r\ttestdata/payloads/payload_odd.bin\treddit\ttestdata/covers/cover_odd_domain.png'
)

# Recover-only fixtures for formats no current conceal option produces:
# the version-1 Reddit carrier and the payload IDAT preceded by a 512 KiB
# all-zero IDAT from the retired platform-padding option. They are
