...
# color-profile preservation and palette validation in both concealment modes.
set -euo pipefail
...
usage() {
    cat <<'EOF'
Usage: tests/run_image_regression_tests.sh [options]

Options:
  --no-build    Accepted for symmetry with the other suites; this one never
                builds, so it is a no-op.
...
# Build indexed samples independently of the decoder, including packed rows.
# Every fixture has valid chunk CRCs so malformed PLTE sizes reach that check.
def indexed_png(path, bit_depth, palette):
    entries = max(1, len(palette) // 3)
    rows = []
    for y in range(68):
        row = bytearray((68 * bit_depth + 7) // 8)
        for x in range(68):
            index = (x + y) % min(entries, 1 << bit_depth)
            bit = x * bit_depth
            row[bit // 8] |= index << (8 - bit_depth - bit % 8)
        rows.append(bytes(row))
    make_png(path, 3, bit_depth, rows, ((b"PLTE", palette),))


def reject_plte(case_name, cover, *command):
    case_dir = WORK / case_name
    case_dir.mkdir()
    result = subprocess.run(
        [str(BIN), *command, str(cover), *([str(WORK / "p.txt")] if command[0] == "conceal" else [])],
        cwd=case_dir,
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
    ...
    if result.returncode == 0 or "invalid PLTE size" not in result.stdout:
        raise AssertionError(f"{case_name}: expected explicit PLTE rejection\n{result.stdout}")
    if list(case_dir.iterdir()):
        raise AssertionError(f"{case_name}: rejection left output files")


for length in (0, 1, 2, 4, 5, 767, 769, 770, 771):
    cover = WORK / f"invalid_plte_{length}.png"
    indexed_png(cover, 8, bytes(length))
    for mode, options in (("default", ()), ("mastodon", ("-m",))):
        reject_plte(f"invalid_plte_{length}_{mode}", cover, "conceal", *options)
print("[PASS] invalid PLTE byte lengths are rejected without output in both concealment modes")

for bit_depth in (1, 2, 4):
    cover = WORK / f"excess_plte_depth_{bit_depth}.png"
    indexed_png(cover, bit_depth, bytes(3 * ((1 << bit_depth) + 1)))
    for mode, options in (("default", ()), ("mastodon", ("-m",))):
        reject_plte(f"excess_plte_depth_{bit_depth}_{mode}", cover, "conceal", *options)
print("[PASS] indexed PLTE entries cannot exceed their bit-depth capacity")

bad_palette = WORK / "invalid_plte_4.png"
reject_plte("invalid_plte_capsize", bad_palette, "capsize")
reject_plte("invalid_plte_reddit", bad_palette, "conceal", "-r")
print("[PASS] capacity inspection and Reddit concealment reject malformed PLTE")

for bit_depth, entries in ((1, 1), (1, 2), (2, 4), (4, 16), (8, 256)):
    palette = b"".join(bytes((i, 255 - i, i ^ 0x55)) for i in range(entries))
    cover = WORK / f"valid_plte_{bit_depth}_{entries}.png"
    indexed_png(cover, bit_depth, palette)
    source_chunks = parse_png(cover)
    source_idat = next(body for kind, body in source_chunks if kind == b"IDAT")
    for mode, options in (("default", ()), ("mastodon", ("-m",))):
        label = f"valid_plte_{bit_depth}_{entries}_{mode}"
        output_chunks = parse_png(conceal(label, cover, *options))
        if output_chunks[0][1][8:10] != bytes((bit_depth, 3)):
            raise AssertionError(f"{label}: original indexed format changed")
        if [body for kind, body in output_chunks if kind == b"PLTE"] != [palette]:
            raise AssertionError(f"{label}: original PLTE changed")
        if next(body for kind, body in output_chunks if kind == b"IDAT") != source_idat:
            raise AssertionError(f"{label}: original pixel IDAT changed")
print("[PASS] valid 1/2/4/8-bit indexed PLTE and pixel IDAT survive both concealment modes")

# Truecolor PNGs may carry a suggested palette; PLTE validation must continue
# to permit these and preserve the exact-copy path for 16-bit pixels.
suggested_palette = b"".join(bytes((i, 255 - i, i ^ 0x55)) for i in range(256))
rgb16_palette = WORK / "rgb16_suggested_palette.png"
make_png(rgb16_palette, 2, 16, rows16, ((b"PLTE", suggested_palette),))
source_idat = next(body for kind, body in parse_png(rgb16_palette) if kind == b"IDAT")
for mode, options in (("default", ()), ("mastodon", ("-m",))):
    label = f"rgb16_suggested_palette_{mode}"
    output_chunks = parse_png(conceal(label, rgb16_palette, *options))
    if output_chunks[0][1][8:10] != bytes((16, 2)):
        raise AssertionError(f"{label}: original RGB16 format changed")
    if next(body for kind, body in output_chunks if kind == b"IDAT") != source_idat:
        raise AssertionError(f"{label}: original pixel IDAT changed")
print("[PASS] valid RGB16 suggested palettes are accepted with unchanged pixel IDAT")
