#include "profileVec.h"

// Default vector, where data is stored in the last IDAT chunk of the PNG cover image.
std::vector<uint_fast8_t>default_vec {
	0x75, 0x5D, 0x19, 0x3D, 0x72, 0xCE, 0x28, 0xA5, 0x60, 0x59, 0x17, 0x98,
	0x13, 0x40, 0xB4, 0xDB, 0x3D, 0x18, 0xEC, 0x10, 0xFA, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0xC6, 0x50, 0x3C, 0xEA, 0x5E, 0x9D, 0xF9,
	0x90, 0x82
};

// COLOR PROFILE FOR PNG / Mastodon, where data need to be stored in the iCCP chunk. 0x1FE
std::vector<uint_fast8_t>mastodon_vec {
	
};

std::vector<std::string> platforms_vec { 
	"X-Twitter", 
	"ImgPile",
	"Mastodon & X-Twitter.", 
	"Mastodon. (Only share this \"file-embedded\" PNG image on Mastodon).", 
	"Reddit. (Only share this \"file-embedded\" PNG image on Reddit).",
	"PostImage",
	"ImgBB", 
	"Flickr"
	};



