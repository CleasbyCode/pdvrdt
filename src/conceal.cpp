...
void validateRedditPreliminaryLimits(
	...
	constexpr std::size_t
		COVER_LIMIT_MIB   = MAX_REDDIT_COVER_IMAGE_SIZE / (1024 * 1024),
		PAYLOAD_LIMIT_MIB = REDDIT_UPLOAD_SIZE_LIMIT / (1024 * 1024);

	const bool cover_too_large = cover_size > MAX_REDDIT_COVER_IMAGE_SIZE;
	const bool payload_too_large = payload_size > REDDIT_UPLOAD_SIZE_LIMIT;
	if (cover_too_large && payload_too_large) {
		throw std::runtime_error(std::format(
			"File Size Error: Cover image exceeds the {} MiB Reddit cover image size limit, "
			"and payload file exceeds Reddit's {} MiB upload size limit.",
			COVER_LIMIT_MIB, PAYLOAD_LIMIT_MIB));
	}
	if (cover_too_large) {
		throw std::runtime_error(std::format(
			"Image File Size Error: Cover image exceeds the {} MiB Reddit cover image size limit.",
			COVER_LIMIT_MIB));
	}
	if (payload_too_large) {
		throw std::runtime_error(std::format(
			"Data File Size Error: Payload file exceeds Reddit's {} MiB upload size limit.",
			PAYLOAD_LIMIT_MIB));
	}
}
...
void concealRedditData(
	...
	// Use the version-2 Argon2id carrier key: checking the known carrier header
	// must incur a memory-hard derivation for each candidate PIN.
	vBytes embedded_png =
