...
[[noreturn]] void reportRedditCapacityFailure(
	...
	std::println(
		...
		carrier.width,
		carrier.height);
	std::println("Compressed payload size: {} bytes.", compressed_size);
	std::println(
		"Theoretical adaptive payload limit for this cover image: {} bytes.",
		carrier.payload_capacity);
	throw std::runtime_error(std::format(
		"Data File Size Error: Compressed payload file size of {} bytes exceeds "
		"the theoretical maximum limit of {} bytes ({} KB) for this cover image.",
		compressed_size,
		carrier.payload_capacity,
...
void concealRedditData(
	...
	// Reuse pdvrdt's normal validation/optimization pass first. The Reddit
	// carrier then decodes those pixels to opaque RGB8 and its fresh encoder
	// intentionally retains no metadata chunks.
	static_cast<void>(optimizeImage(png_vec));
	...
	// This is the only path that prints carrier diagnostics: successful embeds
	// stay concise, and encryption-overhead failures do not masquerade as a
	// compressed-payload capacity failure.
	if (compressed.size() > carrier.payload_capacity) {
		reportRedditCapacityFailure(carrier, compressed.size());
	}
