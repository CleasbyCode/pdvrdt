...
constexpr std::size_t
	...
	REDDIT_PADDING_CHUNK_TOTAL_SIZE  = 0x80000 + PNG_CHUNK_OVERHEAD;
...
[[nodiscard]] std::size_t maximumMastodonCompressedProfileSize(std::size_t optimized_png_size) {
	const auto [output_limit, label] = sizeLimitForOption(Option::Mastodon);
...
void reportOutputOrThrow(
	...
	struct ReportWiper {
		std::string& report;
		~ReportWiper() {
			if (!report.empty()) sodium_memzero(report.data(), report.size());
		}
	} report_wiper{report};
...
void writeRedditPaddingChunk(int fd) {
	...
	static thread_local std::array<Byte, 64 * 1024> zeros{};
...
void writeOutputFile(std::size_t output_size, std::uint64_t& pin, Writer&& writer) {
	...
	struct PinWiper {
		std::uint64_t& pin;
		~PinWiper() { sodium_memzero(&pin, sizeof(pin)); }
	} pin_wiper{pin};
...
[[nodiscard]] vBytes deflateMastodonProfile(
...
void writeMastodonOutput(
	...
	const vBytes compressed_profile = deflateMastodonProfile(
		profile_vec, option, is_compressed, max_compressed_profile_size);
...
void concealData(vBytes& png_vec, Option option, const fs::path& data_file_path) {
	...
	// Owns the PIN until write finishes or any post-encrypt path throws.
	SensitiveU64 pin{encryptCompressedFileToProfile(
		profile_vec,
		data_file.fd(),
		data_file.size(),
		data_filename,
	...
	if (is_mastodon) {
		writeMastodonOutput(png_vec, profile_vec, option, is_compressed, has_bad_dims, pin.value);
	} else {
		writeDefaultOutput(png_vec, profile_vec, option, is_reddit, has_bad_dims, pin.value);
	}
}
