...
void writeRedditPaddingChunk(int fd) {
	...
	while (remaining != 0) {
		...
		writeAllOrThrow(fd, std::span<const Byte>(zeros.data(), chunk_size));
		remaining -= chunk_size;
	}

	writeAllOrThrow(fd, IDAT_REDDIT_CRC);
}
...
void writeOutputFile(std::size_t output_size, std::size_t pin, const std::function<void(int)>& writer) {
	if (!writer) {
...
void concealData(vBytes& png_vec, Option option, const fs::path& data_file_path) {
	...
	const std::size_t pin = encryptCompressedFileToProfile(
		profile_vec,
		data_file_path,
		data_filename,
		option,
	...
	if (is_mastodon) {
		...
		writeOutputFile(output_size, pin, [&](int fd) {
			writeAllOrThrow(fd, std::span<const Byte>(png_vec.data(), MASTODON_INSERT_INDEX));
		...
		writeOutputFile(output_size, pin, [&](int fd) {
			writeAllOrThrow(fd, std::span<const Byte>(png_vec.data(), insert_index));
			if (is_reddit) {
