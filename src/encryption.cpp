...
[[nodiscard]] std::uint64_t getPin() {
	...
	// Pre-reserve capacity so push_back never grows through the SSO→heap
	// transition. Any intermediate allocation would leave copies of partial
	// PIN digits in freed memory beyond the reach of sodium_memzero.
	std::string input;
	input.reserve(MAX_PIN_LENGTH + 1);
	char ch{};
