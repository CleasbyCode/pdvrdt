...
void appendBytes(vBytes& out, std::span<const Byte> bytes, std::string_view error_message) {
	if (bytes.empty()) return;

	const std::size_t base = out.size();
	const std::size_t final_size = checkedAddSize(base, bytes.size(), error_message);
