...
std::uint64_t encryptDataFile(vBytes& profile_vec, vBytes& data_vec, const std::string& data_filename, bool has_mastodon_option) {
	...
	vBytes payload_with_prefix(payload_size + prefix_len);
	payload_with_prefix[0] = static_cast<Byte>(data_filename.size());
	std::memcpy(
		payload_with_prefix.data() + 1,
		data_filename.data(),
		data_filename.size()
	);
	if (payload_size > 0) {
		std::memcpy(
			payload_with_prefix.data() + static_cast<std::ptrdiff_t>(prefix_len),
			data_vec.data(),
			payload_size
		);
		sodium_memzero(data_vec.data(), data_vec.size());
	}
	data_vec = std::move(payload_with_prefix);
