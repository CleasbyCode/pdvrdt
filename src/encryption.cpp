...
struct TermiosGuard {
	...
	explicit TermiosGuard(bool is_tty) {
		...
		if (sigemptyset(&guarded_signals) != 0 ||
			...
			// Prompt and masking writes can raise SIGPIPE when stdout is a closed
			// pipe. Keep it blocked until terminal restoration, including on errors.
			sigaddset(&guarded_signals, SIGPIPE) != 0 ||
...
std::size_t computeRedditEncryptedProfileSize(
	std::size_t prepared_data_size,
	std::size_t filename_length) {

	if (filename_length == 0 ||
		filename_length > static_cast<std::size_t>(std::numeric_limits<Byte>::max())) {
		throw std::runtime_error("Data File Error: Invalid data filename length.");
	}

	constexpr std::size_t FRAME_OVERHEAD =
		STREAM_FRAME_LEN_BYTES + crypto_secretstream_xchacha20poly1305_ABYTES;
	const auto framed_size = [](std::size_t plaintext_size) {
		if (plaintext_size == 0) return std::size_t{0};
		const std::size_t frame_count =
			1 + ((plaintext_size - 1) / STREAM_CHUNK_SIZE);
		if (frame_count >
			(std::numeric_limits<std::size_t>::max() - plaintext_size) /
			FRAME_OVERHEAD) {
			throw std::runtime_error("File Size Error: Encrypted output overflow.");
		}
		return plaintext_size + frame_count * FRAME_OVERHEAD;
	};

	std::size_t profile_size = DEFAULT_OFFSETS.encrypted_file;
	profile_size = checkedAddSize(
		profile_size,
		crypto_secretstream_xchacha20poly1305_HEADERBYTES,
		"File Size Error: Encrypted output overflow.");
	profile_size = checkedAddSize(
		profile_size,
		framed_size(1 + filename_length),
		"File Size Error: Encrypted output overflow.");
	profile_size = checkedAddSize(
		profile_size,
		framed_size(prepared_data_size),
		"File Size Error: Encrypted output overflow.");
...
std::uint64_t deriveCarrierKeyFromPin(const SensitiveU64& pin) {
	// Carrier version 2. These are wire-format parameters, deliberately fixed
	// rather than libsodium's changeable INTERACTIVE presets. The salt cannot be
	// read from the encrypted payload until the carrier has been located, so use
	// a separate, fixed domain salt. This permits cross-image amortization, but
	// every distinct PIN candidate still requires an Argon2id evaluation. Conceal
	// generates a fresh random 64-bit PIN for each image.
	static constexpr Salt DOMAIN_SALT = {
		'p','d','v','r','d','t','c','a','r','r','i','e','r','-','v','2'
	};
	constexpr unsigned long long OPSLIMIT = 2;
	constexpr std::size_t MEMLIMIT = 64ULL * 1024 * 1024;
	...
	if (crypto_pwhash(
			...
			reinterpret_cast<const char*>(pin_bytes.data()), pin_bytes.size(),
			DOMAIN_SALT.data(), OPSLIMIT, MEMLIMIT,
			crypto_pwhash_ALG_ARGON2ID13) != 0) {
		throw std::runtime_error("KDF Error: Unable to derive Reddit carrier key.");
	}
...
std::uint64_t deriveLegacyCarrierKeyFromPin(const SensitiveU64& pin) {
	// Recovery only: version-1 carriers used this inexpensive hash and remain
	// vulnerable to cheap PIN checks. Never use it when creating a new carrier.
