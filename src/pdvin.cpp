//	PNG Data Vehicle _ Insert (for Reddit - pdvin v1.2). Created by Nicholas Cleasby (@CleasbyCode) 24/01/2023
...
// Deflate (compress) iCCP Profile chunk, which include user's data file.
void deflate(std::vector<unsigned char>&);

// Code to compute CRC32 (for iCCP Profile chunk within this program). Taken from: https://www.w3.org/TR/2003/REC-PNG-20031110/#D-CRCAppendix 
...
int main(int argc, char** argv) {
	...
	if (argc == 2 && std::string(argv[1]) == "--info") {
		std::cout << "\nPNG Data Vehicle _ Insert (for reddit - pdvin v1.2). Created by Nicholas Cleasby (@CleasbyCode) 24/01/2023.\n\n" <<
				"Insert file(s) into a PNG image which you can then post on reddit.\n\n" <<
				"Note: This method does not work for Twitter. Please use pdvzip.\n\n" <<	
				"1,048,444 bytes is the filesize limit that you can insert into a single image.\n\n" <<
				"To maximise the amount of data you can embed in your image file, I recommend compressing your\n" << 
				"file(s) to zip/rar formats, etc. Make sure the compressed file does not exceed 1,048,444 bytes.\n\n" <<
				"You can insert up to five files at a time (outputs one image per file).\n\n" <<
				"Use pdvex to extract your file(s) from the image.\n\n";
	}
...
void readFilesIntoVectors(std::ifstream& readImage, std::ifstream& readFile, const std::string& IMAGE_FILE, const std::string& DATA_FILE, const ptrdiff_t& IMAGE_SIZE, const ptrdiff_t& DATA_SIZE, int argc, int sub) {
	...
	const std::string 
		...
		EMBEDDED_IMAGE_FILE = "pdv_img_"+TXT_NUM+".png",
		PNG_SIG = "\x89PNG",
...
// Compress iCCP Profile + user's data file.
void deflate(std::vector<unsigned char>& Vec) {

	// zlib function, see https://zlib.net/
	
	std::vector <unsigned char> Buffer;

	constexpr size_t BUFSIZE = 1032 * 1024; // Size required for deflate. This BUFSIZE covers us up to our file size limit of 1MB. 
						// A lower BUFSIZE results in lost data for larger files.
