...
// Map the project's zlib level choice onto a libdeflate level. libdeflate level
// 9 matches zlib level 9 ratio at a fraction of the time; levels 10-12 are
// "ultra" tiers that cost far more for marginal gains, so BEST -> 9 (not 12).
[[nodiscard]] int libdeflateLevel(Option option, bool is_compressed_file) {
	...
	return 9;      // matches Z_BEST_COMPRESSION intent, far faster
}
