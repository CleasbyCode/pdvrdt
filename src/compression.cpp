...
void writeAllToFd(int fd, std::span<const Byte> buffer) {
	std::size_t written = 0;
	while (written < buffer.size()) {
		const std::size_t remaining = buffer.size() - written;
		...
		const ssize_t rc = ::write(
			fd,
			buffer.data() + static_cast<std::ptrdiff_t>(written),
			chunk_size
		);
		if (rc < 0) {
			if (errno == EINTR) {
			...
			throw std::runtime_error(std::format("Write File Error: Failed to write complete output file: {}", ec.message()));
		}
		if (rc == 0) {
...
[[nodiscard]] std::size_t inflateReserveHint(std::size_t input_size, std::size_t max_output_size) {
	...
	const std::size_t doubled = input_size * 2;
	if (doubled < input_size || doubled > max_output_size) {
		return max_output_size;
	}
	return doubled;
}
...
void zlibDeflate(vBytes& data_vec, Option option, bool is_compressed_file) {
	...
	const uLong bound = compressBound(bounded_input);
