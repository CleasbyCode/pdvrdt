...
// Scrub secret or reversibly-compressed data before releasing its allocation.
class SpanWiper {
	Byte* data_;
	std::size_t size_;
	bool active_{true};
...
public:
	SpanWiper(Byte* data, std::size_t size) noexcept : data_(data), size_(size) {}

	SpanWiper(const SpanWiper&) = delete;
	SpanWiper& operator=(const SpanWiper&) = delete;

	~SpanWiper() {
		if (active_ && data_ != nullptr && size_ != 0) {
...
void inflateDriver(std::span<const Byte> input, std::size_t max_output_size, OutputHandler&& on_output) {
	...
	SpanWiper buffer_wiper{buffer.get(), ZLIB_BUFSIZE};
...
[[nodiscard]] int deflateCompressionLevel(Option option, bool is_compressed_file) {
	...
	// >64 MiB zlib streaming path, level 9 costs far more time for ~1-2% ratio.
...
void libdeflateZlibCompress(std::span<const Byte> input, int level, const DeflateChunkHandler& on_chunk) {
	...
	SpanWiper out_wiper{out.get(), bound};
...
[[nodiscard]] vBytes readWholeFile(int fd, std::size_t size) {
	vBytes buffer(size);
	SpanWiper buffer_wiper{buffer.data(), buffer.size()};
	std::size_t offset = 0;
	while (offset < size) {
...
void deflateDriver(Option option, bool is_compressed_file, const DeflateChunkHandler& on_chunk, RefillInput&& refill_input) {
	...
	SpanWiper output_wiper{output_buffer.get(), ZLIB_BUFSIZE};
...
void zlibDeflateFd(int fd, std::size_t expected_size, Option option, bool is_compressed_file, const DeflateChunkHandler& on_chunk) {
	...
	if (expected_size <= LIBDEFLATE_WHOLE_BUFFER_LIMIT) {
		...
		SpanWiper input_wiper{input.data(), input.size()};
	...
	SpanWiper input_wiper{input_buffer.get(), ZLIB_BUFSIZE};
	...
	deflateDriver(option, is_compressed_file, on_chunk, [&](z_stream& strm) {
		if (strm.avail_in == 0 && !reached_eof) {
			if (input_offset == expected_size) {
				...
				const ssize_t rc = preadRetry(fd, input_buffer.get(), request_size, input_offset);
				if (rc < 0) throwReadError();
				if (rc == 0) throw std::runtime_error("Failed to read full file: partial read");
...
vBytes zlibInflatePrefix(std::span<const Byte> data, std::size_t prefix_size) {
	...
	auto refill_input = [&] {
		if (strm.avail_in != 0 || input_offset >= data.size()) return;
		const std::size_t chunk_size = zlibChunkSize(data.size() - input_offset);
		setZlibInput(strm, data.data() + static_cast<std::ptrdiff_t>(input_offset), chunk_size);
		input_offset += chunk_size;
	};
