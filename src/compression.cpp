...
// Scrubs a byte range on scope exit. Used to keep secret plaintext from
// lingering in freed heap after it has been compressed, mirroring the
// wiping the recover path already performs on the decrypted plaintext.
class SpanWiper {
	Byte* data_;
	std::size_t size_;
...
void zlibDeflateFile(const fs::path& path, Option option, bool is_compressed_file, const DeflateChunkHandler& on_chunk) {
	...
	if (const auto whole_size = wholeBufferEligibleFileSize(path)) {
		vBytes input = readWholeFile(path, *whole_size);
