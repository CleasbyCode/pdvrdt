void startPdv(std::string& image_file_name, std::string& data_file_name, bool isMastodonOption, bool isRedditOption) {
	...
	constexpr uint8_t PNG_MIN_SIZE = 68;
	...
	std::vector<uchar>Image_Vec((std::istreambuf_iterator<char>(image_file_ifs)), std::istreambuf_iterator<char>());
	...
	const uint32_t LAST_SLASH_POS = static_cast<uint32_t>(data_file_name.find_last_of("\\/"));
	...
	const uint32_t FILE_NAME_LENGTH = static_cast<uint32_t>(data_file_name.length());
	...
	std::vector<uchar>File_Vec((std::istreambuf_iterator<char>(data_file_ifs)), std::istreambuf_iterator<char>());

	uint32_t data_file_size = static_cast<uint32_t>(File_Vec.size());
	...
	std::vector<uchar>Profile_Data_Vec = {
	...
	if (isRedditOption) {
		...
		constexpr uchar IDAT_REDDIT_CRC_BYTES[4]{ 0xA3, 0x1A, 0x50, 0xFA };

		std::vector<uchar>Idat_Reddit_Vec = { 0x00, 0x08, 0x00, 0x00, 0x49, 0x44, 0x41, 0x54 };
	...
	valueUpdater(Profile_Data_Vec, profile_data_vec_size_index, static_cast<uint32_t>(Profile_Data_Vec.size()), bits);
	...
	const uint32_t
		PROFILE_DEFLATE_SIZE = static_cast<uint32_t>(Profile_Data_Vec.size()),
		CHUNK_SIZE = isMastodonOption ? PROFILE_DEFLATE_SIZE + 9 : PROFILE_DEFLATE_SIZE + 5,
		CHUNK_INSERT_INDEX = isMastodonOption ? 33 : static_cast<uint32_t>(Image_Vec.size() - 12);
	...
	if (isMastodonOption) {
		...
		std::vector<uchar>Iccp_Chunk_Vec = { 0x00, 0x00, 0x00, 0x00, 0x69, 0x43, 0x43, 0x50, 0x69, 0x63, 0x63, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
		...
		const uint32_t ICCP_CHUNK_CRC = crcUpdate(&Iccp_Chunk_Vec[CHUNK_START_INDEX], CHUNK_SIZE, buf_index, initialize_crc_value);

		uint32_t iccp_crc_insert_index = CHUNK_START_INDEX + CHUNK_SIZE;
		...
		std::vector<uchar>Idat_Chunk_Vec = { 0x00, 0x00, 0x00, 0x00, 0x49, 0x44, 0x41, 0x54, 0x5F, 0x00, 0x00, 0x00, 0x00 };
		...
		const uint32_t IDAT_CHUNK_CRC = crcUpdate(&Idat_Chunk_Vec[CHUNK_START_INDEX], CHUNK_SIZE, buf_index, initialize_crc_value);

		uint32_t idat_crc_insert_index = CHUNK_START_INDEX + CHUNK_SIZE;
	...
	image_file_size = static_cast<uint32_t>(Image_Vec.size());
