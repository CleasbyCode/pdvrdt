void startPdv(const std::string& IMAGE_FILENAME, std::string& data_filename, bool isMastodonOption, bool isRedditOption) {
	...
	uint32_t PROFILE_DATA_VEC_SIZE = encryptFile(Profile_Data_Vec, File_Vec, data_filename);
	...
	const uint32_t PROFILE_DATA_VEC_DEFLATE_SIZE = deflateFile(Profile_Data_Vec, DATA_FILE_EXTENSION);
	...
	const uint8_t PROFILE_DATA_INSERT_INDEX = isMastodonOption ? 0x0D : 9;
	...
	const uint32_t EMBEDDED_IMAGE_FILE_SIZE = static_cast<uint32_t>(Image_Vec.size());
