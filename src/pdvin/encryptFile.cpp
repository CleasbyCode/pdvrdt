uint32_t encryptFile(std::vector<uint8_t>&Profile_Data_Vec, std::vector<uint8_t>&File_Vec, std::string& data_filename) {

	std::random_device rd;
 	std::mt19937 gen(rd());
	std::uniform_int_distribution<unsigned short> dis(1, 255); 
	
	constexpr uint16_t PROFILE_XOR_KEY_INSERT_INDEX = 0x197;

	constexpr uint8_t 
		XOR_KEY_LENGTH = 12,
		PROFILE_NAME_INDEX = 0x65;

	const uint32_t DATA_FILE_SIZE = getVecSize(File_Vec);
	...
	while (DATA_FILE_SIZE > index_pos) {
		...
		Profile_Data_Vec.emplace_back(File_Vec[index_pos++] ^ data_filename[name_key_pos++]);
	}	

	Profile_Data_Vec[PROFILE_NAME_INDEX - 1] = static_cast<uint8_t>(DATA_FILENAME_LENGTH);
