...
static_assert(MAX_COVER_IMAGE_SIZE == 4ULL * 1024 * 1024,
	"displayInfo() quotes a 4 MiB cover-image limit; keep it in step with MAX_COVER_IMAGE_SIZE.");
static_assert(MAX_REDDIT_COVER_IMAGE_SIZE == 16ULL * 1024 * 1024,
	"displayInfo() quotes a 16 MiB Reddit cover-image limit; keep it in step with MAX_REDDIT_COVER_IMAGE_SIZE.");
...
static_assert(MAX_COVER_DIMENSION == 4096,
	"displayInfo() quotes a 4096x4096 cover-dimension limit; keep it in step with MAX_COVER_DIMENSION.");
static_assert(MAX_REDDIT_COVER_DIMENSION == 8192,
	"displayInfo() quotes an 8192x8192 Reddit cover-dimension limit; keep it in step with MAX_REDDIT_COVER_DIMENSION.");
...
──────────────────────────
  ...
  -r (Reddit) : Creates an adaptive spatial "file-embedded" RGB PNG for Reddit.
      ...
      Use "pdvrdt capsize my_image.png" to inspect a Reddit cover before choosing a payload.
...
[[nodiscard]] ProgramArgs parseCapsizeArgs(int argc, char** argv, const std::string& usage) {
	if (argc != 3 || argAt(argc, argv, 2).empty()) {
		dieUsage(usage);
	}
...
std::optional<ProgramArgs> ProgramArgs::parse(int argc, char** argv) {
	...
	if (mode == "capsize") return parseCapsizeArgs(argc, argv, usage);
