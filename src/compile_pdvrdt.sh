...
# release  -- the hardened binary to install.
# sanitize -- ASan/UBSan build for fuzzing and regression triage.
BUILD_MODE="${PDVRDT_BUILD_MODE:-release}"
...
case "$BUILD_MODE" in
  release|sanitize) ;;
  *)
    echo "Unsupported PDVRDT_BUILD_MODE: $BUILD_MODE (expected release or sanitize)." >&2
    exit 1
    ;;
esac
...
BUILD_KEY="${COMPILER_NAME}-${OPT_LEVEL#-}-${BUILD_MODE}"
...
echo ""
echo "Compilation successful. Executable '$OUTPUT' is up to date. (mode: $BUILD_MODE)"
if [[ "$BUILD_MODE" == "release" ]]; then
  echo ""
  echo "ASan/UBSan build, for fuzzing and regression triage:"
  echo "  PDVRDT_BUILD_MODE=sanitize PDVRDT_OUTPUT=pdvrdt-san ./compile_pdvrdt.sh"
  echo "  bash tests/run_golden_tests.sh --bin ./pdvrdt-san"
fi
echo ""
