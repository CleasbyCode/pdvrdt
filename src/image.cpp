...
[[nodiscard]] bool looksLikePdvrdtIdat(std::span<const Byte> chunk_data) {
	...
	return
		...
		std::ranges::equal(
			profile.subspan(DEFAULT_OFFSETS.kdf_metadata + KDF_MAGIC_OFFSET, KDF_METADATA_MAGIC_V2.size()),
...
void removeExistingPdvrdtIdatChunks(vBytes& image_file_vec) {
	...
	requirePngSignature(image_file_vec, "PNG Error: Invalid PNG signature.");

	const PngChunkView ihdr = readPngChunk(
		image_file_vec,
		PNG_HEADER_SIZE,
		"PNG Error: Corrupt IHDR chunk.",
		"PNG Error: Corrupt IHDR chunk length.",
		"PNG Error: Corrupt IHDR chunk CRC."
	);
	if (ihdr.length != IHDR_DATA_SIZE || ihdr.type != TYPE_IHDR) {
	...
	while (pos < image_file_vec.size()) {
		...
		const bool keep_chunk = chunk.type != TYPE_IDAT || !looksLikePdvrdtIdat(chunk.data);
		if (keep_chunk) {
			if (write_pos != pos) {
