...
constexpr std::size_t MAX_LODEPNG_DECODE_BYTES = 256ULL * 1024 * 1024;
...
void preflightPngDecode(std::span<const Byte> png) {
	...
	if (std::memcmp(png.data(), PNG_SIG.data(), PNG_HEADER_SIZE) != 0) {
		throw std::runtime_error("PNG Error: Invalid PNG signature.");
	}

	const std::size_t pos = PNG_HEADER_SIZE;
	const std::size_t ihdr_len = getValue(png, pos, 4);
	const std::uint32_t ihdr_type = static_cast<std::uint32_t>(getValue(png, pos + 4, 4));
	if (ihdr_len != IHDR_DATA_SIZE || ihdr_type != TYPE_IHDR) {
		throw std::runtime_error("PNG Error: Missing or corrupt IHDR chunk.");
	}
	requireSpanRange(png, pos, CHUNK_OVERHEAD + IHDR_DATA_SIZE, "PNG Error: Corrupt IHDR chunk length.");

	const std::size_t ihdr_data = pos + 8;
	const std::uint32_t stored_crc = static_cast<std::uint32_t>(getValue(png, ihdr_data + IHDR_DATA_SIZE, 4));
	const std::uint32_t computed_crc = lodepng_crc32(png.data() + static_cast<std::ptrdiff_t>(pos + 4), IHDR_DATA_SIZE + 4);
	if (stored_crc != computed_crc) {
		throw std::runtime_error("PNG Error: Corrupt IHDR chunk CRC.");
	}

	const std::uint32_t width = static_cast<std::uint32_t>(getValue(png, ihdr_data, 4));
	const std::uint32_t height = static_cast<std::uint32_t>(getValue(png, ihdr_data + 4, 4));
	const Byte bit_depth = png[ihdr_data + 8];
	const Byte color_type = png[ihdr_data + 9];
	const Byte compression_method = png[ihdr_data + 10];
	const Byte filter_method = png[ihdr_data + 11];
	const Byte interlace_method = png[ihdr_data + 12];
...
void removeExistingPdvrdtIdatChunks(vBytes& image_file_vec) {
	...
	requireSpanRange(image_file_vec, pos, CHUNK_OVERHEAD + IHDR_DATA_SIZE, "PNG Error: Corrupt IHDR chunk.");
	...
	const std::uint32_t stored_ihdr_crc = static_cast<std::uint32_t>(getValue(image_file_vec, pos + 8 + IHDR_DATA_SIZE, 4));
	const std::uint32_t computed_ihdr_crc = lodepng_crc32(
		image_file_vec.data() + static_cast<std::ptrdiff_t>(pos + 4),
		IHDR_DATA_SIZE + 4
	);
	if (stored_ihdr_crc != computed_ihdr_crc) {
		throw std::runtime_error("PNG Error: Corrupt IHDR chunk CRC.");
	}

	pos += CHUNK_OVERHEAD + IHDR_DATA_SIZE;
	...
	while (pos < image_file_vec.size()) {
		...
		const bool keep_chunk = chunk_type != TYPE_IDAT || !looksLikePdvrdtIdat(chunk_data);
		if (keep_chunk) {
			if (write_pos != pos) {
