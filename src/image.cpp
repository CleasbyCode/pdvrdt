...
// sBIT's length is fixed by the image's color type (PNG spec 11.3.3). Zero for a
// color type that has no valid sBIT, which readRequiredIhdr()/the geometry
// preflight have already ruled out by the time this runs.
[[nodiscard]] constexpr std::size_t sbitLengthForColorType(Byte color_type) {
	switch (color_type) {
		case 0: return 1;  // greyscale
		case 2: return 3;  // truecolor
		case 3: return 3;  // indexed -- significance of the palette's RGB samples
...
// byte range and all. lodepng never looks at them
// (LODEPNG_NO_COMPILE_ANCILLARY_CHUNKS) and readPngChunk() only checks the CRC,
// so without this a cover can hand pdvrdt a malformed chunk that it then
// faithfully re-emits -- producing an image that fails PNG validators through no
// fault of the payload, and giving the cover an unvalidated channel for planting
// arbitrary bytes in whatever gets shared. Each chunk's length (and, where it is
// cheap, its contents) is checked against the spec here, once, in the preflight
// both output paths run.
void requireValidColorMetadataChunk(const PngChunkView& chunk, Byte color_type, Byte bit_depth) {
	const auto reject = [](std::string_view detail) {
		throw std::runtime_error(std::format("PNG Error: Invalid color metadata. {}", detail));
	};
	const auto requireLength = [&](std::size_t expected, std::string_view name) {
		if (chunk.length != expected) {
			reject(std::format("{} chunk must be {} bytes.", name, expected));
		}
	};

	switch (chunk.type) {
		case TYPE_CHRM: requireLength(32, "cHRM"); break;
		case TYPE_GAMA: requireLength(4,  "gAMA"); break;
		case TYPE_CICP: requireLength(4,  "cICP"); break;
		case TYPE_MDCV: requireLength(24, "mDCv"); break;
		case TYPE_CLLI: requireLength(8,  "cLLi"); break;
		case TYPE_SRGB:
			// Spelt out rather than requireLength() + a read, so the bounds of the
			// data access are visible at the point it happens.
			if (chunk.length != 1) {
				reject("sRGB chunk must be 1 byte.");
			} else if (chunk.data[0] > 3) {
				reject("sRGB rendering intent must be 0-3.");
			}
			break;
		case TYPE_SBIT: {
			const std::size_t expected = sbitLengthForColorType(color_type);
			if (expected == 0 || chunk.length != expected) {
				reject(std::format(
					"sBIT chunk must be {} bytes for color type {}.",
					expected, static_cast<unsigned>(color_type)));
			}
			// A palette's entries are 8-bit samples whatever the image's bit
			// depth, which is only the width of the index.
			const Byte sample_bits = (color_type == INDEXED_PLTE) ? Byte{8} : bit_depth;
			for (const Byte significant_bits : chunk.data) {
				if (significant_bits == 0 || significant_bits > sample_bits) {
					reject(std::format(
						"sBIT significant-bit counts must be 1-{}.",
		...
		case TYPE_ICCP: {
			...
			const auto separator = std::ranges::find(chunk.data, Byte{0});
			const std::size_t name_length =
				static_cast<std::size_t>(separator - chunk.data.begin());
			if (separator == chunk.data.end() || name_length == 0 || name_length > 79) {
				reject("iCCP profile name must be 1-79 bytes followed by a null separator.");
			} else if (chunk.length < name_length + 3) {
				reject("iCCP chunk carries no compressed profile.");
			} else if (chunk.data[name_length + 1] != 0) {
				reject("iCCP compression method must be 0 (deflate).");
			}
			break;
		}
		default: break;
...
void validateStaticPngChunks(
	std::span<const Byte> png,
	std::size_t first_chunk_pos,
	Byte color_type,
	Byte bit_depth) {
	...
	forEachChunkToIend(png, first_chunk_pos, [&](const PngChunkView& chunk) {
		...
		if (isColorMetadataChunk(chunk.type)) {
			requireValidColorMetadataChunk(chunk, color_type, bit_depth);
		}
...
void requireCoverDimensionsWithinLimit(std::span<const Byte> png, std::uint32_t max_dimension) {
	...
	const std::uint32_t
		width  = getValue(ihdr.data, 0),
		height = getValue(ihdr.data, 4);

	if (width > max_dimension || height > max_dimension) {
		throw std::runtime_error(std::format(
			"Image File Error: Cover image dimensions of {}x{} exceed the maximum of {}x{} for this conceal mode.",
...
void preflightPngDecode(std::span<const Byte> png, std::uint32_t max_cover_dimension) {
	// Three parts. The cover-dimension limit is mode policy and comes first so it
	// owns the error message for an oversized cover. The geometry bound lives in
	// png_utils so the recover-only binary can reach it without linking this file;
	// the chunk policy below is cover-image policy and stays here.
	requireCoverDimensionsWithinLimit(png, max_cover_dimension);
	requirePngGeometryWithinDecodeLimit(png);

	const PngChunkView ihdr = readRequiredIhdr(png);
	validateStaticPngChunks(
		png,
		ihdr.offset + ihdr.total_size,
		ihdr.data[9],   // color type
		ihdr.data[8]);  // bit depth
}
...
bool optimizeImage(vBytes& image_file_vec) {
	...
	preflightPngDecode(image_file_vec, MAX_COVER_DIMENSION);
...
void prepareCoverForRedditCarrier(vBytes& image_file_vec) {
	preflightPngDecode(image_file_vec, MAX_REDDIT_COVER_DIMENSION);
	// A stale pdvrdt IDAT payload in a reused cover sits after the real image
	// data, so leaving it would make the concatenated IDAT stream look like
	// trailing garbage to the decoder. Removing it by chunk here is exact and
	// costs a walk; letting decodeCarrier() discover it costs a whole extra
	// inflate to find where the real stream ends.
	removeExistingPdvrdtIdatChunks(image_file_vec);
}
