...
[[nodiscard]] bool looksLikePdvrdtIccp(std::span<const Byte> chunk_data) {
	...
	// as a new cover, while a genuine ICC profile must remain intact. Shares the
	// recover-side prefix inflater (see tryExtractMastodonProfileFromIccp) so
	// both paths agree on exactly which iCCP chunks count as a pdvrdt payload.
