...
void requireSpanRange(std::span<const Byte> data, std::size_t index, std::size_t length, std::string_view message) {
...
void closeFdOrThrow(int& fd) {
	...
	while (::close(fd) < 0) {
		...
		const std::error_code ec(errno, std::generic_category());
		throw std::runtime_error(std::format("Write File Error: Failed to finalize output file: {}", ec.message()));
	}
	fd = -1;
}
...
[[nodiscard]] bool hasKdfMetadata(std::span<const Byte> profile, const ProfileOffsets& offsets) {
	...
	return matchesBytes(magic, std::to_array<Byte>({'K', 'D', 'F', '2'})) &&
		...
		offsets.encrypted_file < profile.size();
}
...
EmbeddedProfile locateEmbeddedData(const vBytes& png_vec) {
	...
	std::optional<vBytes> default_profile{};
	std::optional<vBytes> mastodon_profile{};
	bool has_iend = false;
	...
	if (mastodon_profile) {
		return EmbeddedProfile{ .profile = std::move(*mastodon_profile), .is_mastodon = true };
	}
	if (default_profile) {
		return EmbeddedProfile{ .profile = std::move(*default_profile), .is_mastodon = false };
	}
	throw std::runtime_error("Image File Error: This is not a pdvrdt image.");
}
