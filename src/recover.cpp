...
[[nodiscard]] fs::path safeRecoveryPath(std::string decrypted_filename) {
	...
	if (!pathEntryExistsOrThrow(candidate, "Write File Error: Failed to check output path")) {
		return candidate;
	}
	...
	for (std::size_t i = 1; i <= 10000; ++i) {
		...
		if (!pathEntryExistsOrThrow(next, "Write File Error: Failed to check output path")) {
			return next;
		}
	}
