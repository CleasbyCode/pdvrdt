...
std::optional<EmbeddedProfile> locateMetadataEmbeddedData(const vBytes& png_vec) {
...
void recoverData(vBytes& png_vec) {
	...
	if (auto embedded = locateMetadataEmbeddedData(png_vec)) {
		...
		// it. That is the property being bought: a wrong PIN and an ordinary PNG
		// are indistinguishable to anyone but the holder, so they share one
		// message below.
		getPin(recovery_pin);

		const auto reddit_profile =
			extractRedditPngPayload(png_vec, deriveCarrierKeyFromPin(recovery_pin));
		if (!reddit_profile) {
