...
[[nodiscard]] bool hasKdfMetadata(std::span<const Byte> profile, const ProfileOffsets& offsets) {
	...
	return spanHasRange(profile, offsets.kdf_metadata, KDF_METADATA_REGION_BYTES) &&
		bytesEqualAt(profile, offsets.kdf_metadata + KDF_MAGIC_OFFSET, KDF_METADATA_MAGIC_V2) &&
...
void recoverData(vBytes& png_vec) {
	...
	PlaintextWiper plaintext_wiper{png_vec};
	// Wipe the plaintext recovered filename. The std::move into safeRecoveryPath
	// transfers the heap to fs::path internal storage (which we cannot portably
	// wipe), but the wiper still scrubs *result on exception paths between here
	// and the move.
	FilenameWiper filename_wiper{*result};
